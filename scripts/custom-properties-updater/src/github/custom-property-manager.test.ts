...
describe('CustomPropertyManager', () => {
    ...
    describe('getCustomProperty', () => {
        test('gets existing custom properties', async () => {
            ...
            const properties = await customPropertyManager.getCustomProperty('org1', 'customProp1');

            expect(properties.customPropertyName).toBe('customProp1');
            expect(properties.data).toHaveLength(2);
            expect(properties.data).toContainEqual({repository: "repo1", oldValue: "old value 1", newValue: ""})
            expect(properties.data).toContainEqual({repository: "repo2", oldValue: "old value 3", newValue: ""})
        });
        ...
        test('updates properties', async () => {
            ...
            expect(properties.customPropertyName).toBe('customProp1');
            expect(properties.data).toHaveLength(2);
            expect(properties.data).toContainEqual({repository: "repo1", oldValue: "new value 1", newValue: ""})
            expect(properties.data).toContainEqual({repository: "repo2", oldValue: "old value 3", newValue: ""})
        });
